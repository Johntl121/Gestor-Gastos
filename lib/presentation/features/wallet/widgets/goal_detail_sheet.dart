...
class GoalDetailSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Selector<WalletProvider, GoalEntity?>(
      selector: (context, provider) => provider.goals.where((g) => g.id == goal.id).firstOrNull,
      builder: (context, updatedGoal, child) {
        ...
        final goalColor = isCompleted ? const Color(0xFFFFD700) : currentGoal.color;

        // Datos de la cuenta alcancía
        final vaultAccount = currentGoal.accountId != null
            ? walletProvider.accounts.where((a) => a.id == currentGoal.accountId).firstOrNull
            : null;
          ...
          children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    ...
                    showModalBottomSheet(
                      ...
                      builder: (ctx) => GoalFormSheet(goalToEdit: currentGoal),
                    );
                  },
                ),
              ],
            ...
            Row(
              ...
              children: [
                _buildStatChip(
                  "Has ahorrado",
                  "$currencySymbol ${currentGoal.currentAmount.toStringAsFixed(2)}",
                  Colors.green,
                  isDarkMode,
                ),
                _buildStatChip(
            ...
            if (currentGoal.deadline != null) ...[
              _buildDeadlineInfo(currentGoal, currencySymbol, goalColor, isDarkMode),
              const SizedBox(height: 14),
            ],
  ...
  Widget _buildStatChip(String label, String value, Color color, bool isDarkMode) {
    return Expanded(
      child: Container(
  ...
  Widget _buildDeadlineInfo(GoalEntity goal, String currency, Color goalColor, bool isDarkMode) {
  ...
  void _confirmPurchase(BuildContext context, GoalEntity currentGoal) {
    ...
    int selectedCategoryId = currentGoal.categoryId ?? AppConstants.shoppingCategoryId;

    // Nombre de la cuenta alcancía
    final vaultAccount = currentGoal.accountId != null
        ? provider.accounts.where((a) => a.id == currentGoal.accountId).firstOrNull
        : null;
              ...
              children: [
                ...
                Container(
                  ...
                  child: DropdownButton<int>(
                    value: AppCategories.expenseCategories.containsKey(selectedCategoryId)
                        ? selectedCategoryId
                        ...
                        child: Row(
                          children: [
                            Icon(AppCategories.getIcon(entry.key),
                                size: 18, color: Color(entry.value['color'] as int)),
  ...
  void _confirmDeleteGoal(BuildContext context, GoalEntity currentGoal) {
    ...
    if (currentGoal.currentAmount > 0) {
      final provider = Provider.of<WalletProvider>(context, listen: false);
      int selectedRefundAccount = provider.accounts.isNotEmpty
